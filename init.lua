...
local function get_user_selection()
  local buf = 0
...
local function get_end_translation(text)
...
local function open_blb(term, translation)
  local formatted_term = url_encode(term)
  local url = 'https://www.blueletterbible.org/search/preSearch.cfm?Criteria=' .. formatted_term .. '&t=' .. translation
  vim.fn.jobstart({ 'xdg-open', url }, { detach = true })
end

vim.api.nvim_create_user_command('BLB', function()
  local text = get_user_selection()

  if not text then
    return
  end

  local new_text, translation = get_end_translation(text)
  if translation then
    text = new_text
  else
    translation = M.config.translation
  end
  open_blb(text, translation)
end, {})
